    ...
    return (
        <main>
            ...
            <Overlay onBackgroundClick={() => setPage(null)} shown={page !== null}>
                {pages.map((e, i) => <PagePopup key={i} disease={e} shown={page === i} handleClose={() => setPage(null)} />)}
            </Overlay>
        </main>
    );
}
...
function PagePopup({ disease: { name, content }, shown, handleClose }: { disease: PageData, shown: boolean, handleClose: React.MouseEventHandler }) {
    const [displayed, setDisplayed] = useState(false);
    const [animated, setAnimated] = useState(false);
